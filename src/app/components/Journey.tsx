    ...
    for (let attempt = 0; attempt < 2 && !data; attempt++) {
      ...
      try {
        const res = await fetchT("/api/interview", {
          ...
          // Send the WHOLE profile back — the brain re-asks anything it cannot see.
          profile: {
            role: base.role, name: base.name, contact: base.contact,
                    ...
                    {profile.wovenLines.length > 0 && (
                      <div className="jn-pl">
                        ...
                        {profile.wovenLines.map((ln, i) => {
                          ...
                          // The "- " marks a bullet in the data; the preview shows
                          // the bullet, not the character (as the PDF template does).
                          const shown = ln.trim().replace(/^[-•]\s*/, "");
                          return (
                            <div key={i} className={`jn-p-line editable${head ? " head" : ""}`} title="✎" onClick={() => editLine(i)}>
                              {head ? <b>{shown}</b> : `• ${shown}`}
                            </div>
                          );
                        })}
                      </div>
