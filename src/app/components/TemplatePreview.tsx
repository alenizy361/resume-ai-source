...
/**
 * Hoisted out of the render, with its captured value passed in.
 *
 * Declared inside `TemplatePreview` this was a NEW component type every render, so React
 * remounted its subtree instead of updating it. The subtlety is that it CLOSED OVER the
 * `t` prop — hoisting it blindly compiles to `Cannot find name 't'`, which is the useful
 * kind of failure. `accent` is now a prop, which is what the closure was doing implicitly.
 */
function H({ accent, children }: { accent: string; children: React.ReactNode }) {
  return (
    <div style={{ color: accent, fontWeight: 700, fontSize: 11, letterSpacing: 1, textTransform: "uppercase", borderBottom: `2px solid ${accent}`, paddingBottom: 3, marginBottom: 6, marginTop: 12 }}>{children}</div>
  );
}
...
export default function TemplatePreview({ t }: { t: TemplateData }) {
  ...
  const body = (
    <>
      ...
      <H accent={t.accent}>Experience</H>
