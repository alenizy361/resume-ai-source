...
| P2-2 | 4 | Duplicate and tailor | `DONE` | `duplicateServerResume`/`POST /api/resume` gave this a server-side primitive but had zero client callers. Added a "Duplicate → tailor for a job" action to `/builder`'s start screen (`BuilderStart.tsx`): clones a saved resume's full local record under a fresh id (title suffixed "(copy)"/"(نسخة)"), and lands on the target step so the job details can be edited for the new application while the original stays untouched. The clone is picked up by the existing server mirror (`useServerSync`) the same as any new resume — no direct call to the duplicate endpoint was needed. |
| P2-3 | 5 | Saudi occupation knowledge base | `PARTIAL`, improved this pass — 10 of 29 resolvable occupations now have a `RolePack` (`rolePacks.ts`): added Teacher, Civil Engineer, Pharmacist and Lawyer, chosen because each already had a `CredentialRule` in `countryRules.ts` with no pack to pair it with — the product could name their licence but had no duties/skills/keywords to offer alongside it. Credential titles and issuers in the new packs are copied verbatim from the matching rule so the two files cannot drift apart. Re-examined the fabrication concern raised earlier against `countryRules.ts`'s OWN stated doctrine — a pack's content is offered as an unconfirmed suggestion the user accepts or edits, never asserted as fact ("the confirmation step is the guarantee, not the data's provenance," `countryRules.ts:27-28`) — the same safety net already covering the original 6 packs, so extending it to 4 more occupations is consistent with the existing risk posture, not a new category of risk. `ops/rolepacks.test.mjs` (184 assertions, up from ~170) enforces the no-invented-numbers rule on all ten. 19 occupations remain without a pack, and `countryRules.ts` still covers Saudi Arabia only despite `COUNTRY_NAMES` resolving 9 other markets — those rules assert jurisdiction-specific regulatory facts rather than generic duties, a materially different risk, and were not attempted. |
...
| P3-2 | 8 | Interview preparation | `DONE` — was left `PARTIAL` citing the F-18 CV-hookup fix as if it were a residual gap; it isn't. `/interview` (202 lines) generates 8 scored Q&A with model answers; `/interview-live` (486 lines) runs a full scored mock interview with camera, TTS, live transcription and a paywall gate; both bilingual, both CV-aware via `MyCvPicker`, both server-rendered for SEO. `ops/mycvs.test.mjs`/`ops/mycvs.browser.mjs` cover the CV hand-off. No gap named anywhere in `known-issues.md`. |
| P3-3 | 9 | LinkedIn improvement | `DONE` — same correction as P3-2. `/linkedin` (150 lines) returns headline/About/skills/tips via `/api/tools`, explicitly forbids inventing employers or achievements, answers in the CV's language (F-18), and is server-rendered for SEO. |
