...
| P2-3 | 5 | Saudi occupation knowledge base | `PARTIAL`, improved across two passes — 14 of 29 resolvable occupations now have a `RolePack` (`rolePacks.ts`): Teacher, Civil Engineer, Pharmacist, Lawyer, then Physician, Dentist, Physiotherapist and Laboratory Technologist, which completes every occupation the `sa-scfhs-registration` credential rule itself names (radiographer/nurse/pharmacist already had packs; physician/dentist/physiotherapist/lab-technologist did not). Same discipline as the first four: credential titles/issuers copied verbatim from the matching rule, content offered as an unconfirmed suggestion never asserted as fact — `countryRules.ts`'s own doctrine, not a new risk category. `ops/rolepacks.test.mjs` (232 assertions) enforces no-invented-numbers on all fourteen. 15 occupations remain without a pack (respiratory therapist, medical technologist, paramedic, the four non-civil engineering disciplines, auditor, the three teacher subject-variants, the three lawyer variants); `countryRules.ts` still covers Saudi Arabia only despite `COUNTRY_NAMES` resolving 9 other markets — those assert jurisdiction-specific regulatory facts rather than generic duties, a materially different risk, and remain untouched. |
