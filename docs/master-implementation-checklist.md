...
| P2-3 | 5 | Saudi occupation knowledge base | `PARTIAL` — confirmed, not stale. Only 6 of 29 resolvable occupations (`occupations.ts`) have a matching `RolePack` (`rolePacks.ts`); the other 23 fall back to generic, non-occupation-specific suggestions. `countryRules.ts` has credential rules for Saudi Arabia only, despite `COUNTRY_NAMES` already resolving 9 other markets — IT, sales, admin, logistics and hospitality have zero encoded rules in any country. All 11 existing rules are `status: "encoded"`, none `"verified"` — `ops/verify-rules.mjs` exists and is idle because verifying them means opening SCFHS/ETEC/SOCPA's own sites, which this sandbox's network policy 403s. Closing the coverage gap is data-entry against an existing shape, but each pack asserts specific professional duties/credentials as fact — the same fabrication risk `DRAFTING_DOCTRINE` exists to prevent elsewhere in this product — so it was not attempted blind in this pass; it needs either real domain sourcing or an explicit sign-off to publish unverified. |
